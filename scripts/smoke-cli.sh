...
set +e
"$NODE" "$CLI" convert --unknown >"$TEMP_ROOT/invalid-output" 2>"$TEMP_ROOT/invalid-error"
CODE=$?
set -e
test "$CODE" = 1
test ! -s "$TEMP_ROOT/invalid-output"
test "$(cat "$TEMP_ROOT/invalid-error")" = 'Unknown option: --unknown'
