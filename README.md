...
### Accounting coverage and replay

The additive `receipt.accounting` evidence counts observed assistant messages by
node and model. Each input, output, cache-read, cache-write, and USD cost component
has an `observed` count and a `total` (`null` when unobserved; zero when reported
as zero). Compare each count to `messages` at the same scope. Family, node, and
model summaries overlap: do not add them together. Totals with incomplete
coverage are observed subtotals, not complete usage. Transcript messages can
include local mirrors; they are not a provider-request ledger. Costs are runtime
reported and billing-unreconciled. Capture `complete` does not certify accounting
...
```bash
openclaw-atif convert --graph retained/graph.json --bundle-root retained --output replay
```

Replay verifies the retained source hashes and preserves partial capture;
relocation does not require the original OpenClaw state. Use the same exporter
build for reproducibility. Hashes detect changed bytes, not forged evidence.
Legacy migration provenance is not rerun by `convert`. The graph cannot restore
deleted generations or hidden ACP content. Redacted public bundles still contain
task/session content: they are not automatically safe to publish.
